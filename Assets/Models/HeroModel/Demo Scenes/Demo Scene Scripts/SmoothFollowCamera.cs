...
//		float anpha = target.transform.rotation.y - transform.rotation.y;
		transform.position = follower.Update(targetPoint + offset, Time.deltaTime);
//		transform.position = transform.position + new Vector3 ((float)4 * (float)System.Math.Sin (anpha * (float)3.14 / 180),
//		                                                       2,  (-1) * (float)4 * (float)System.Math.Cos (anpha * (float)3.14 / 180));
